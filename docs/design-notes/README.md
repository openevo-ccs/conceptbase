...
| [`lpmr-management-app-spec.md`](lpmr-management-app-spec.md) | **Dormant — its premise (extending the ConceptBase Explorer app) no longer holds; the Explorer was retired, see README's Roadmap.** Full planning spec for a new GitHub-OAuth-gated **LPMR Management App**: explore/review/comment/flag/tag any OECB-compatible LPMR with real GitHub identity, optionally invoke a `curriculum-agents` persona over SAIA for a first-pass AI review, and turn any of it into a human-reviewed PR — never a silent write. Kept as historical planning context, not an active spec; §16's open decisions were never resolved and Phase 0 never started. |
