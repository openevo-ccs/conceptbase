...
| `OPENEVO-FOUNDATIONAL-COMPETENCIES` | *(post-supersession — no numeric block; ids mint directly as* `OE-COMPETENCY-openevo-foundational-competencies-<slug>`, *record filenames continue `001100`–`001113` for readability only, not reservation)* | 14 records (systems-thinking, critical-thinking, and 12 others), migrating `openevo-graph/nodes/competencies.json`'s legacy competency nodes; per RFC-0021 (**proposed**, not yet accepted). Third OpenEvo-authored competency vocabulary. Deliberately placed one full block past RFC-0011's claimed upper bound (`001099`) to avoid a fourth collision on top of the three already on this table. **Governance self-contradiction found while drafting this row, not resolved here:** this table already says CompetencyBase's id scheme was "superseded (2026-08-16)" and that new vocabularies should mint per CompetencyBase's own `GOVERNANCE.md` rather than reserve a block here — but CompetencyBase's own `GOVERNANCE.md` (as of RFC-0021's drafting) still describes the *old* numeric block-allocation scheme as current and doesn't mention the supersession at all. This row follows the newer (post-supersession) scheme as the empirically correct one — real records for `OPENEVO-CORE-COMPETENCIES`/`CRITICAL-AI-LITERACY` already use slug ids — and is added here anyway, since this table still calls itself "the single collision-free registry"; a maintainer should reconcile the two docs. |
